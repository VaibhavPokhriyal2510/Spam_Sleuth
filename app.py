...
if selected == "Home":
    ...
    st.markdown(
        ...
        """, unsafe_allow_html=True)
    st.markdown('<p class="spam-headers">For better appearance change system\'s theme to light.Steps are given below:<br><br>1.Select Start<br> 2.Go to Settings.<br> 3.Select Personalization.<br> 4.Select Colors. <br>5.In the list for Choose your color, select light.',unsafe_allow_html=True)
    st.markdown('<p class="spam-headers">1.Spam Mail Detector:The Spam Mail Detector is a powerful tool designed to identify and classify spam emails. It leverages advanced algorithms and machine learning techniques to analyze various aspects of an email, including the header, content, and attachments. By using the Spam Mail Detector, forensic investigators can efficiently filter out spam emails from legitimate ones, saving time and resources in their investigations.<br><br> 2.Email Header Analyzer:The Email Header Analyzer is a valuable tool for forensic investigators to extract and analyze critical information from email headers. Email headers contain vital metadata, including sender and recipient details, timestamps, message IDs, and authentication results. By using the Email Header Analyzer, investigators can uncover valuable insights, such as the source of the email, authentication status, and potential indicators of malicious activity.<br><br>3.Attachment Analysis: It is a crucial aspect of email forensics, which involves examining and extracting information from email attachments to gather evidence, understand the context, or identify potential threats. It plays a vital role in investigations related to cybercrime, corporate espionage, data breaches, and other malicious activities.<br><br> 4.IP Tracker:The IP Tracker tool enables forensic investigators to track and trace the origin and geographical location of an IP address. By analyzing the IP address associated with suspicious activities, investigators can gain valuable information about the potential source of an attack or unauthorized access. The IP Tracker tool provides geolocation data, ISP information, and other relevant details that assist in identifying and apprehending cybercriminals.<br><br>5.Domain Lookup:The Domain Lookup tool is an essential resource for forensic investigators to gather information about an IP address\'s ownership and historical data. By performing an IP lookup, investigators can identify the organization or individual associated with an IP address, along with contact details and network information. This helps investigators in establishing connections, conducting further investigations, and building a comprehensive profile of potential suspects or sources of malicious activity.<br><br> With the combined capabilities of the Spam Mail Detector, Email Header Analyzer, IP Tracker,Domain Lookup tools and Attachment Analysis forensic investigators have a robust suite of resources at their disposal to conduct thorough investigations, uncover evidence, and track down cybercriminals. These tools streamline the forensic process, enhance efficiency, and provide valuable insights that contribute to the successful resolution of cases.',unsafe_allow_html=True)
